...
# Route for handling questionnaire form submission from support workers
...
def submit_questionnaire():
    ...
    # Collect all 15 question answers from the form
    answers = {}
    for i in range(1, 16):
        q = f'question{i}'
    ...
    # Create new questionnaire answer record with all fields
...
# AJAX endpoint to get list of patients for dropdown
@questionnaire_bp.route('/ajax_get_patients')
@login_required
def ajax_get_patients():
    # Get all patients and return as JSON for populating dropdown
    patients = Patient.query.all()
    return jsonify([{'id': p.id, 'name': p.name} for p in patients])

# AJAX endpoint to get available report dates for a selected patient
@questionnaire_bp.route('/ajax_get_dates_by_patient/<int:patient_id>')
@login_required
def ajax_get_dates_by_patient(patient_id):
    # Query for all distinct report dates for the specified patient
    dates = (
    ...
    # Format dates as strings and return as JSON
    return jsonify([d.report_date.strftime('%Y-%m-%d') for d in dates])

# AJAX endpoint to get full report data for a patient on a specific date
@questionnaire_bp.route('/ajax_get_report/<int:patient_id>/<string:report_date>')
@login_required
def ajax_get_report(patient_id, report_date):
    from datetime import datetime
    # Parse and validate the report date
    try:
        parsed_date = datetime.strptime(report_date, '%Y-%m-%d').date()
    except ValueError:
        return jsonify({'error': 'Invalid date format'}), 400

    # Find the questionnaire record for this patient and date
    ...
    # Return formatted report data with all question answers
    return jsonify({
